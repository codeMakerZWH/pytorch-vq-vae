...
from dataLoader.step2_DataLoader import myDataset, VQStep2Dataset
...
def train(epoch, training_loader, model, VQVAE_Model, save_path,total_loss=0.0, total_psnr=0.0, total_ssim=0.0):
    ...
    for sharp, blur in iter(train_data):
        ...
        vq_loss, blur_quantize, _, _ = model._vq_vae(blur_vq_output_eval)
        ...
        loss = recon_error + vq_loss + 0.01*codebook_loss
        loss.backward()
        optimizer.step()
    ...
    if total_psnr > 0.0:
        ...
        print(f"Epoch {epoch+1}/{num_epochs}, Loss: {total_loss/train_data_size/batch_size:.4f}, PSNR: {mean_psnr:.4f}, SSIM: {mean_ssim:.4f}")
    else:
...
if __name__ == '__main__':
    name = 'VQ-VQ_loss' + train_data_path.split('\\')[-2]
